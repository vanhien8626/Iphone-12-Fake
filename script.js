$(function(){
  ...
  const globalState = {
    ...
    dateTime: {
      ...
      date: ['Sunday', 'Saturday', 'Friday', 'Thursday', 'Wednesday', 'Tuesday', 'Monday'],
    },
    bateriaBaja: false,
    draggScreen: false
  }
  ...
  $.fn.extend({
    ...
    Lịch: function(config){
      config = jQuery.extend({
        fecha: new Date(),
        diaCompleto: false
      }, config);
      let mes = globalState.dateTime.month[config.fecha.getMonth()];
      let dateMes = new Date(config.fecha.getFullYear(), (config.fecha.getMonth() + 1), 0).getDate();
...
<div class="tablaContent"></div>
      ...
      globalState.dateTime.date.map(dia => header.append(`<div class="diaName">${config.diaCompleto ? dia : dia.charAt(0)}</div>`))
      for (var k = 0; k <= primerDia; k++) {
        content.prepend('<div></div>');
      }
      for (let index = 1; index <= dateMes; index++) {
    ...
    fechaIcono: function(config){
      ...
      let hoy = config.fecha.getDate();
      let dia = globalState.dateTime.date[config.fecha.getDay()];
...
<path d="M62 38l-7.259-9.86L46 36"></path>
  ...
  function renderizarUI(){
    ...
    if ($('.wrapperApps .app[data-app="widgetFullLịch"]').length) {
      //Preparamos el widget del calendario
      $('.wrapperApps .app[data-app="widgetFullLịch"] .icono').append('<div class="eventos"><p>lịch"></div>');
      //Creamos el calendario del widget
      $('.wrapperApps .app[data-app="widgetFullLịch"] .icono .LịchWrapper').Lịch();
    }
    //Si existe el icono dinamico del calendario
    if ($('.wrapperApps .app.LịchDinamico').length) {
      //Icono dinamico del calendario
      $('.wrapperApps .app.LịchDinamico .icono').fechaIcono();
    }
